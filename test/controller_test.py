...
def test_constructor():
    c = Controller(player, menu, listener, mocks.Timer())
...
def test_modeShouldReturnMainModeDependingOnTimerMode():
    ...
    c = Controller(player, menu, listener, timer)
...
def test_playerMode_selectShouldNotDelegateToMenu():
    menu = mocks.Menu()
    c = Controller(player, menu, listener, mocks.timerInMainMode())
    c.select()
...
def test_menuMode_selectShouldDelegateToMenu():
    menu = mocks.Menu()
    c = Controller(player, menu, listener, mocks.timerInMenuMode())
    c.select()
...
def test_menuMode_moveByShouldDelegateToMenu():
    ...
    c = Controller(player, mocks.Menu(), listener, timer)
    c.exitMenuMode()
